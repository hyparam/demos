...
export function squirrelingDataFrame({
  ...
  return {
    ...
    async fetch({ rowStart, rowEnd, columns, signal }) {
      ...
      // Resolve all cells in parallel so fast columns (e.g. iceberg data) land
      // immediately while slow columns (e.g. LLM UDFs) stream in as they
      // complete. Dispatching `resolve` per cell lets HighTable re-render
      // incrementally instead of waiting for the whole fetch.
