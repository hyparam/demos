...
export default function Page({ metadata, name, from, byteLength, setError }: PageProps): ReactNode {
  ...
  useEffect(() => {
    ...
    try {
      ...
      void collectFeatures(rowGen, abortController.signal)
        .then(result => {
    ...
    <div className='sql-container'>
      ...
      <div className='example-queries'>
        {exampleQueries.map(({ label, query: q }) =>
          <button key={label} className={query === q ? 'active' : ''} onClick={() => handleQueryChange(q)}>{label}</button>
        )}
      </div>
    </div>
...
function isGeometry(value: unknown): value is Geometry {
  return typeof value === 'object' && value !== null && 'type' in value && geometryTypes.has((value as Geometry).type)
}
...
async function collectFeatures(
  ...
  // Collect all rows
  const rows: { columns: string[], values: Record<string, unknown> }[] = []
  for await (const row of rowGen) {
    if (signal.aborted) break
    const values: Record<string, unknown> = {}
    for (const col of row.columns) {
      values[col] = await row.cells[col]()
    }
    rows.push({ columns: row.columns, values })
  }
  ...
  const geomColumn = firstRow.columns.find(col => isGeometry(firstRow.values[col]))

  if (!geomColumn) return []

  // Convert rows to features
  const features: MapFeature[] = []
  for (const row of rows) {
    const geometry = row.values[geomColumn]
    if (!isGeometry(geometry)) continue

    const properties: Record<string, unknown> = {}
    for (const col of row.columns) {
      if (col !== geomColumn) {
        properties[col] = row.values[col]
      }
    }
